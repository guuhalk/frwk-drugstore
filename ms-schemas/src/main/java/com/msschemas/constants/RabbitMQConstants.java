package com.msschemas.constants;

public class RabbitMQConstants {
	
	public static final String QUEUE_USER = "USER";
	public static final String QUEUE_USER_RETURN = "USER_RETURN"; 
	public static final String QUEUE_CATEGORY = "CATEGORY";
	public static final String QUEUE_PHARMACIES = "PHARMACIES";
	public static final String QUEUE_FINANCIAL_INSTITUTIONS = "FINANCIAL_INSTITUTIONS";
	public static final String QUEUE_REQUESTS = "REQUESTS";
	public static final String QUEUE_PRODUCTS = "PRODUCTS";
	public static final String QUEUE_ACCOUNTS = "ACCOUNTS";

}
